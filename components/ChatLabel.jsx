...
const ChatLabel = ({ openMenu, setOpenMenu, id, name }) => {
  const { fetchUsersChats, chats, setselectedChats,setChats } = useAppContext();
  ...
  const renameHandler = async () => {
    ...
    console.log("Renaming chat", id, "to", newName); // ✅ Add this
    ...
    const { data } = await axios.post("/api/chat/rename", {
      ...
      if (data.success) {
        // 🔁 Update chat name in context without full refresh
        ...
        setselectedChats(updatedChats.find((chat) => chat._id === id));
        setChats(updatedChats); // ✅ CORRECT
      } else {
...
rounded-1g"
          ...
          <div
            ...
            className="flex items-center gap-3 hover:bg-white/10 px-3 py
rounded-1g"
          >
